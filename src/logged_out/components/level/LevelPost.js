...
class LevelPost extends PureComponent {
  componentDidMount() {
        ...
        <div className={classes.levelContentWrapper}>
          <Grid container spacing={5}>
            ...
            <Grid item md={3}>
              ...
              {otherArticles.map(levelPost => (
                <Box key={levelPost.id} mb={3}>
                  <LevelCard
                    title={levelPost.title}
                    snippet={levelPost.snippet}
                    date={levelPost.date}
                    url={`${levelPost.url}${levelPost.params}`}
                  />
                </Box>
              ))}
            </Grid>
...
export default withStyles(styles, { withTheme: true })(LevelPost);
